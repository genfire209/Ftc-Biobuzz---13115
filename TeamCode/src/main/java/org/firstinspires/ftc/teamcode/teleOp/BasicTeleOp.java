...
// Standard robot-centric mecanum drivetrain (no IMU/field-centric
// dependency -- kept simple on purpose). Device names below must exactly
// match the names given to the four drive motors in the Driver Hub's
// robot configuration.
//
// Direction settings confirmed via systemTest/WheelDirectionTest
// (isolated single-wheel test) after the robot was rewired: frontLeft/
// backLeft need REVERSE, frontRight/backRight stay at default (FORWARD).
// Do not change without re-running that test on this robot.
